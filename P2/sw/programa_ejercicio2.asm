...
main:
  ...
  ###### PRUEBA DE LOS RIESGOS DEL EJERCICIO 1 ######
  ...
  add $t4, $t2, $t2     # 2
  sw $t1, 24($zero)     # sw $r9,  24($r0) -> data[24] =  2
  lw $t3, 24($zero)     # 2


  ###### PRUEBA DE LA INSTRUCCION BRANCH EJERCICIO 2 ######
  
  
  # instruccion numero 0XC4
  addi $t1, $t2, 1   # $t1 = r9 <- 2
  beq $t4, $t1, target  # efectivo, debería ir a target

  add $t3, $t2, $t1     # r11 <- 1 + 1 = 2. No se tiene que ejecutar
  sub $t5, $t3, $t1     # r13 <- 4 - 2 = 2. No se tiene que ejecutar
  addi $t5, $t5, 1      # r13 <- 2 + 1 = 3. No se tiene que ejecutar


  target:
  add $t1, $t4, $t4     # r9 = 4 
  beq $t4, $t1, target2 # no efectivo
  
  sub $t4, $t2, $t1     # r12 <- 1 - 4 = -3. Este si se debe ejecutar
  
  target2:
  addi $t6, $zero, 32     # r14 = 32
  
  lw $t1, 44($zero)       # r9 <- 32
  beq $t1, $t6, target3   # efectivo

  add $t2, $t4, $t1       # r10 <- -3 + 32 = 29. No se tiene que ejecutar

  target3:
  addi $t3, $zero, 3      # r11 = 3
  addi $t5, $t5, -23      # r13 = 26 - 23 = 3
  lw $t5, 24($zero)       # r13 = 2
  beq $t3, $t5, target4   # no efectivo
  
  sub $t2, $t1, $t1       # r10 <- t1 - t1 = 0. Esta si se debe ejecutar
